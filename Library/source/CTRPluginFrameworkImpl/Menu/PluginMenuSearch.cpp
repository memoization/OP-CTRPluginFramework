...
#define HIDE_REGIONS 1
    ...
    void    PluginMenuSearch::_ListRegion(void)
    {
        ...
        u32         save_addr;
        int         i;
        Region      bakRegion = _memoryRegions.SelectedItem && _regionsList.size() > static_cast<size_t>(_memoryRegions.SelectedItem) ? _regionsList[_memoryRegions.SelectedItem] : (Region){0};
        Result      ret;
